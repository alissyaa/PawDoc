...
<body>
    ...
    if ($stmt = $conn->prepare($sql)) {
        ...
        if ($stmt->fetch()) {
            ...
            do {
                echo "<div class='card'>";
                echo "<p class='date'>" . htmlspecialchars($tanggal_konsultasi) . "</p>";
                echo "<h1 class='title'>" . htmlspecialchars($nama_kucing) . "</h1>";
                echo "<p class='subtitle'>" . htmlspecialchars($penyakit) . "</p>";
                echo "<button class='detail-button'>detail</button>";
                echo "</div>";
            } while ($stmt->fetch());
        } else {
